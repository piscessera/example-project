...
## Automation สำหรับสร้าง detailed design

โปรเจกต์นี้มี skill `/create-detailed-design` (`.claude/skills/create-detailed-design/SKILL.md`) สำหรับรับ topic ที่มี requirement spec อยู่แล้วใน `docs/01-requirements/01-spec/` (และ architecture, API/Database spec, user journey ถ้ามี) มาสร้างเอกสาร Detailed Design แบบ **conceptual** (ไม่ผูกมัดกับ technical stack เฉพาะเจาะจง เช่น ภาษาโปรแกรม, framework, database engine, HTTP method/protocol) ที่ลงรายละเอียดลำดับขั้นตอนการทำงานภายในของแต่ละ operation/สถานการณ์หลัก ต่อยอดจาก conceptual component ใน architecture และ operation ใน API spec ประกอบด้วย pre-condition, main flow, alternate/exception flow, post-condition และ **ต้องมี sequence flow แบบ mermaid `sequenceDiagram` เป็นอย่างน้อยต่อทุก operation/สถานการณ์ที่ทำ** เก็บไว้ที่ไฟล์เดียวที่ `docs/02-design/02-technical/{YYYYMMDD}-{NN}-{topic-slug}-detailed-design.md` skill นี้ทำงานต่อยอดจากเอกสารที่มีอยู่เท่านั้น (ถ้ายังไม่มี spec ของ topic นั้นจะแนะนำให้รัน `/create-requirement` ก่อน และถ้ายังไม่มี architecture หรือ API/Database spec จะถามผู้ใช้กลับพร้อมตัวเลือกอย่างน้อย 3 แนวทางว่าจะทำต่อแบบไหน) และจะถามผู้ใช้กลับพร้อมตัวเลือกอย่างน้อย 3 แนวทางเมื่อเนื้อหาไม่ชัดเจน เมื่อเรียกซ้ำและพบว่า topic นั้นมี detailed design document อยู่แล้วจะแก้ไขไฟล์เดิมในที่เดิม (ไม่สร้างไฟล์ซ้ำ) และก่อนลงมือสร้างไฟล์จริงจะสรุปแผนให้ผู้ใช้ยืนยันก่อนเสมอ แล้วมอบหมายงานเขียนไฟล์จริงให้ subagent `detailed-design-writer` (`.claude/agents/detailed-design-writer.md`) ใช้ skill นี้แทนการสร้างเอกสาร detailed design ด้วยมือ
